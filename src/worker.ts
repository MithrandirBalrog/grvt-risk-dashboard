...
function copyUpstreamHeaders(upstreamHeaders: Headers): Headers {
...
async function proxyApiKeyLogin(request: Request): Promise<Response> {
...
async function handleDevLogin(request: Request): Promise<Response> {
  ...
  if (accountId) {
    headers.set('X-Grvt-Account-Id', accountId)
  }
...
async function routeApi(request: Request): Promise<Response> {
  ...
  if (pathname.startsWith('/api/auth/')) {
    ...
    if (isApiKeyLogin) {
      return proxyApiKeyLogin(request)
    }
